...
class PagesController < ApplicationController
  def home
    if user_signed_in?
      current_participant = Participant.new(:user_id => current_user.id)
  ...
  def leaderlookup
	if user_signed_in?
	  ...
	  if @is_admin or @is_leader
		...
		trainee = Participant.new(:participant_id =>user_id)
  ...
  def teamtotals
	if user_signed_in?
		...
		if is_admin
			...
			@total_amount = Participant.new(:user_id => current_user.id).sum_donations
		end
	end
  end
